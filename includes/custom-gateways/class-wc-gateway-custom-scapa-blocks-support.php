...
class WC_Gateway_Custom_Scapa_Blocks_Support extends AbstractPaymentMethodType {

	/**
	 * Initializes the payment method type.
	 */
	public function initialize() {
		$scapa_gateway_settings        = get_option( 'woocommerce_scapa_settings', array() );
		$custom_scapa_gateway_settings = get_option( "woocommerce_{$this->name}_settings", array() );

		$this->settings = wp_parse_args( $custom_scapa_gateway_settings, $scapa_gateway_settings );
	...
	public function get_payment_method_script_handles() {
		$script_asset_path = plugins_url( "/assets/js/blocks/frontend/blocks/{$this->name}.asset.php", WC_SCAPA_MAIN_FILE );
		...
		$script_url = plugins_url( "/assets/js/blocks/frontend/blocks/{$this->name}.js", WC_SCAPA_MAIN_FILE );
		...
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( "wc-{$this->name}-blocks", 'woo-scapa', );
		}
	...
	public function get_payment_method_data() {
		...
		if ( empty( $payment_icons ) ) {
			$payment_icons = array( 'scapa' );
		}

		$payment_icons_url = array();
		foreach ( $payment_icons as $payment_icon ) {
			$payment_icons_url[] = WC_HTTPS::force_https_url( plugins_url( "assets/images/{$payment_icon}.png", WC_SCAPA_MAIN_FILE ) );
		}
