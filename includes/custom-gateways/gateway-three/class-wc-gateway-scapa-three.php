...
class WC_Gateway_Scapa_Three extends WC_Gateway_Custom_Scapa {
	 ...
	 * @var array
	 */
	public $scapa_settings;

	/**
	 * WC_Gateway_Scapa_Three constructor.
	 */
	public function __construct() {
		...
		if ( empty( $gateway_title ) ) {
			$gateway_title = __( 'Three', 'woo-scapa' );
		}

		$this->method_title       = sprintf( __( 'Scapa - %s', 'woo-scapa' ), $gateway_title );
		...
		$this->scapa_settings = get_option( 'woocommerce_scapa_settings', '' );
		...
		$this->testmode = $this->scapa_settings['testmode'] === 'yes' ? true : false;
		...
		$this->test_public_key = $this->scapa_settings['test_public_key'];
		$this->test_secret_key = $this->scapa_settings['test_secret_key'];

		$this->live_public_key = $this->scapa_settings['live_public_key'];
		$this->live_secret_key = $this->scapa_settings['live_secret_key'];

		$this->saved_cards = $this->scapa_settings['saved_cards'] === 'yes' ? true : false;
